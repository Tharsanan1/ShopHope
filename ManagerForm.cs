...
namespace ShopHope
{
    public partial class ManagerForm : Form
    {
        ...
        private ManagerForm()
        {
            ...
            ProfileViewPanel.Visible = false;
            employeeDataGridView.ReadOnly = false;
            passwordTxt.PasswordChar = '\u25CF';


            viewStockDataGridView.AutoGenerateColumns = false;
            DataGridViewButtonColumn buttonColoumn1 = new DataGridViewButtonColumn();
            buttonColoumn1.HeaderText = "Offer";
            buttonColoumn1.Text = "delete";
            buttonColoumn1.UseColumnTextForButtonValue = true;
            viewStockDataGridView.Columns.Add(buttonColoumn1);
            viewStockDataGridView.Width += 100;
            viewStockDataGridView.CellClick += ViewStockDataGridView_CellClick;
            viewStockDataGridView.Location = new Point(420,220);



        }

        public void checkNameLbl()
        {
            ...
            try
            {
                ...
                {
                    nameLbl.Text = dataReader.GetString("userName");
                }
                conn.Close();
            }
            catch (Exception ex)
        ...
        private void ViewStockDataGridView_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex == 8)
            {
                ...
                {
                    conn.Open();
                    MySqlCommand command = new MySqlCommand("UPDATE shophope.stocks SET offerDate = '' WHERE stockId = '" + viewStockDataGridView.Rows[e.RowIndex].Cells[0].Value.ToString() + "'", conn);
                    MySqlDataReader dataReader = command.ExecuteReader();
                    while (dataReader.Read())
                    {
                    ...
                    StockManagementSystem.checkOfferClose();
                    viewStockDataGridView.Visible = false;
                    fillViewStock();
        ...
        {
            if (e.RowIndex >= 0 && e.ColumnIndex == 4 && e.RowIndex<attandanceDataGridView.Rows.Count-1)
            {
        ...
        public void fillAttandancedataGridView()
        {
            if (attandanceDataGridView.Visible == true)
            {
                ...
                attandanceDataGridView.Visible = false;
                foreach (DataGridViewRow row in attandanceDataGridView.Rows)
                {
                    if(row.Index == attandanceDataGridView.Rows.Count-1) {
                        continue;
                    }
                    MySqlConnection conn = Connection.getConnection();
                    try
                    {
                        conn.Open();
                        MySqlCommand command = new MySqlCommand("UPDATE shophope.employeetable SET todayOverTime = '"+row.Cells[2].Value.ToString()+"', presence = '"+row.Cells[3].Value.ToString()+"' WHERE userName = '"+row.Cells[1].Value.ToString()+"'", conn);
                        MySqlDataReader dataReader = command.ExecuteReader();
                        while (dataReader.Read())
                        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    while (dataReader.Read())
                    {
                        ...
                        row.Cells[7].Value = dataReader.GetString("returnQuantity");
        ...
        public void createEmployeeDetail() {
            ...
            else
            {
                ...
                try
                {
                    ...
                    while (dataReader.Read())
                    {
                        int index = employeeDataGridView.Rows.Add();
                        employeeDataGridView.Rows[index].Cells[0].Value = dataReader.GetString("userId");
                        employeeDataGridView.Rows[index].Cells[1].Value = dataReader.GetString("userName");
                        employeeDataGridView.Rows[index].Cells[2].Value = dataReader.GetString("nIC");
                        employeeDataGridView.Rows[index].Cells[3].Value = dataReader.GetString("emailAdress");
                        employeeDataGridView.Rows[index].Cells[4].Value = dataReader.GetString("adress");
                        employeeDataGridView.Rows[index].Cells[5].Value = dataReader.GetString("phoneNum");
                        employeeDataGridView.Rows[index].Cells[6].Value = dataReader.GetString("post");
        ...
        private void createProfileView() {
            if(ProfileViewPanel.Visible == true) {
                ProfileViewPanel.Visible = false;
            }
            else {
                attandanceDataGridView.Visible = false;
        ...
        {
            foreach (char c in phoneNumtxt.Text)
            {
                if (!char.IsDigit(c))
                {
                    phoneNumtxt.Text = phoneNumtxt.Text.Substring(0, phoneNumtxt.Text.Length - 1);
                    phoneNumtxt.SelectionStart = phoneNumtxt.Text.Length;
                }
            }
        }
        ...
        {
            if(nameTxt.Text.Length>0 && addressTxt.Text.Length>0 && ageTxt.Text.Length>0 && passwordTxt.Text.Length>0 && mailTxt.Text.Length>0 && phoneNumtxt.Text.Length>0 && nicTxt.Text.Length==10) {
                if(passwordTxt.Text.Length<6) {
                    MessageBox.Show("Password strength not enough");
                    return;
                }
                else {
                    MySqlConnection conn = Connection.getConnection();
                    ...
                    {
                        conn.Open();
                        MySqlCommand command = new MySqlCommand("UPDATE shophope.employeetable SET userName = '"+nameTxt.Text+"', passWord = '"+passwordTxt.Text+"',emailAdress = '"+mailTxt.Text+"', nIC = '"+nicTxt.Text+"', adress = '"+addressTxt.Text+"', age = '"+ageTxt.Text+"', phoneNum = '"+phoneNumtxt.Text+"' WHERE userId = '1'", conn);
                        MySqlDataReader dataReader = command.ExecuteReader();
                        while (dataReader.Read())
                        {
        ...
        private void nicTxt_TextChanged(object sender, EventArgs e)
        {
                ...
                {
                    nicTxt.Text = nicTxt.Text.Substring(0, nicTxt.Text.Length - 1);
                    nicTxt.SelectionStart = nicTxt.Text.Length;
                }
        }
        private void createOfferList() {
            ...
            try
            {
                ...
                while (dataReader.Read())
                {
                    ...
                    {
                        if (int.Parse(DateTime.Today.ToString("yyyyMMdd")) < int.Parse(dataReader.GetString("offerDate")))
                        {
                            offerStocks += "\n";
                            offerStocks += dataReader.GetString("stockId");
                        }
                    }
                }
            ...
            MessageBox.Show("Offer created stocks IDs: "+offerStocks);
        }
