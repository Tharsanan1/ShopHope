...
namespace ShopHope
{
    public partial class SalesManForm : Form
    {
        ...
        List<string> quantityPurchasedList;
        private SalesManForm(string name)
        {
            ...
            quantityPurchasedList = new List<string>();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                while (dataReader.Read())
                {
                    ...
                    nameList.Add(nameComboBox.Text);
                    quantityPurchasedList.Add(numberOfStocksForPurchase.Text);
                    priceList.Add(temp);
                }
            }
            catch (Exception ex)
        ...
        private void lastBillBtn_Click(object sender, EventArgs e)
        {
            if (Bill.getBillList().Count > 0)
            {
                try
                {
                    ...
                    nameList.Clear();
                    priceList.Clear();
                    quantityPurchasedList.Clear();
                    Bill temp = Bill.getbill();
                    Console.WriteLine("nbill index: " + Bill.getBillList().IndexOf(temp));
                    for (int i = 0; i < temp.getIdlist().Count; i++)
                    {
                        DataGridViewRow row = (DataGridViewRow)billDataGridView.Rows[0].Clone();
                        row.Cells[0].Value = temp.getIdlist()[i];

                        row.Cells[3].Value = temp.getPriceList()[i];
                        row.Cells[1].Value = temp.getQuantityList()[i];
                        billDataGridView.Rows.Add(row);
                    }
                    DataGridViewRow row1 = (DataGridViewRow)billDataGridView.Rows[0].Clone();
                    row1.Cells[0].Value = "Total";
                    row1.Cells[3].Value = temp.getTotal();
                    billDataGridView.Rows.Add(row1);
                }
                catch (Exception ex)
                {
                ...
                MessageBox.Show("No bill in memory");
            }
        }
        ...
        private void deleteBtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                priceList.RemoveAt(selectedRow);
                nameList.RemoveAt(selectedRow);
                quantityPurchasedList.RemoveAt(selectedRow);
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message);
        ...
        private void finishBtn_Click(object sender, EventArgs e)
        {
            ...
            if (!totalPrice.ToString().Equals("0")) {
                new Bill(nameList.ToList(), quantityPurchasedList.ToList(), priceList.ToList(), totalPrice);
            }
